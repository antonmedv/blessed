...
Textarea.prototype._incCursor = function (val = 1) {
  this._setCursor(this.cursorPosition + val);
};

Textarea.prototype._decCursor = function (val = 1) {
  this._setCursor(this.cursorPosition - val >= 0 ? this.cursorPosition - val : 0);
};

Textarea.prototype._setCursor = function (val = 1) {
...
Textarea.prototype._updateCursor = function(get) {
  ...
  cx = lpos.xi + this.ileft + this.cursorPosition;
    ...
    if (key.name === 'backspace') {
      if (this.value.length) {
        if (this.screen.fullUnicode) {
          if (unicode.isSurrogate(this.value, this.cursorPosition - 2)) {
            // || unicode.isCombining(this.value, this.value.length - 1)) {
            this.value = this.value.slice(0, this.cursorPosition - 2) + this.value.slice(this.cursorPosition);
            this._decCursor(2);
          } else {
      ...
      if (!/^[\x00-\x08\x0b-\x0c\x0e-\x1f\x7f]$/.test(ch)) {
        this.value = this.value.slice(0, this.cursorPosition) + ch + this.value.slice(this.cursorPosition);
        this._incCursor();
      }
    }
